...
app.get("/api/smashings", (req, res) => {
  ...
  //   essentially what the result set will be when the query finishes)
    Smashing.find(q, function(err, result) {
      if(!err){
        res.json(result);
      }
    });
...
app.post("/api/smashing", (req, res) => {
  ...
  console.log(req.body.smashingText);
  console.log(req.body.smashingText.length);
  console.log(countLetters(req.body.smashingText));
