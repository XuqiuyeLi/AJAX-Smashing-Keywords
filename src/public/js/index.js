...
function refreshResults() {
  ...
  const queryString =Array.prototype.filter.call(inputs, ele => ele.value.length > 0).map(ele => `${ele.name}=${ele.value}`).join('&');
...
function postSmashing() {
  ...
  const text = document.getElementById("userSmashing").value;
  console.log(text); 
...
function main() {
  ...
  filterBtn.addEventListener('click', function(evt){
    // prevent the page refreshing itself
    evt.preventDefault();
    refreshResults();
  });
  ...
  const SendDataBtn = document.querySelector('#userInput input[type="submit"]');
  console.log(SendDataBtn);
  SendDataBtn.addEventListener('click', function(evt){
    // prevent the page refreshing itself
    evt.preventDefault();
    postSmashing();
    refreshResults();
